...
const TransactionTable: React.FC = () => { 
 ...
 const filteredTransactions = isLoading ? [] : transaction?.filter(tx =>
  ...
  return (
    <>
      <div className="bg-white dark:bg-card rounded-2xl shadow-xl p-4 overflow-x-auto border transition-shadow duration-200">
    ...
    <input
        ...
        <table className="min-w-full text-sm">
          ...
          {transaction.length <=0 ? <center>Data Not FOund</center> : 
          ...
          <tbody>
            ...
            {isLoading ? (
              ...
              filteredTransactions?.map((tx) => (
                <tr 
                  key={tx.id} 
                  className={`border-b hover:bg-blue-50 dark:hover:bg-blue-900 cursor-pointer transition-colors duration-150 rounded-lg`}
                  onClick={() => handleTransactionClick(tx)}
                >
                  <td className="py-2 px-2">{tx.title}</td>
                  <td className="py-2 px-2">{tx.createdAt ? String(tx.createdAt) : ""}</td>
                  <td className={`py-2 px-2 font-medium ${tx.type === 'CASH_IN' ? 'text-green-600' : 'text-red-500'}`}>{tx.type === 'CASH_IN' ? 'Cash In' : 'Cash Out'}</td>
